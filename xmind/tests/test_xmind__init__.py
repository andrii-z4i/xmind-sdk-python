...
class TestXmindInit(base.Base):
    ...
    def getLogger(self):
        if not getattr(self, '_logger', None):
            self._logger = logging.getLogger('xmindInit')
        return self._logger

    def test_excessive_parameters(self):
        _workbook_loader = self._init_patch_with_name('_workbook_loader', 'xmind.WorkbookLoader')
        _workbook_saver = self._init_patch_with_name('_workbook_saver', 'xmind.WorkbookSaver')

        with self.assertRaises(TypeError) as _ex:
            load("test", "test")
        self.getLogger().debug("Got an exception: '%s'", _ex.exception.args[0])
        self.assertTrue(_ex.exception.args[0].find("%s() takes" % "load") != -1)

        with self.assertRaises(TypeError) as _exNone:
            load()
        self.getLogger().debug("Got an exception: '%s'",_exNone.exception.args[0])
        self.assertTrue(_exNone.exception.args[0].find("%s() missing" % "load") != -1)

        with self.assertRaises(TypeError) as _ex:
            save("test", "test", "test")
        self.getLogger().debug("Got an exception: '%s'", _ex.exception.args[0])
        self.assertTrue(_ex.exception.args[0].find("%s() takes" % "save") != -1)

        with self.assertRaises(TypeError) as _exNone:
            save()
        self.getLogger().debug("Got an exception: '%s'", _exNone.exception.args[0])
        self.assertTrue(_exNone.exception.args[0].find("%s() missing" % "save") != -1)
    ...
    def test_load(self):
        ...
        _workbook_loader = self._init_patch_with_name('_workbook_loader', 'xmind.WorkbookLoader', return_value=_workbook_loader_obj)
    ...
    def test_save(self):
        ...
        _workbook_saver = self._init_patch_with_name('_workbook_saver', 'xmind.WorkbookSaver', return_value=_workbook_saver_obj)
